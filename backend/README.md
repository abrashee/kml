...
## Quality Gates
- Local default validation (fast): `mvn validate`
- CI-quality validation (includes SpotBugs with baseline filter): `mvn -Pci-quality verify`
- SpotBugs baseline file: `config/spotbugs-baseline-exclude.xml`

The CI profile fails on SpotBugs issues that are not present in the baseline filter.
